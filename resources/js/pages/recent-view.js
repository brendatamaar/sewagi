    ...
    const recentView = {
        ...
        initialize: function() {
            ...
            $(document).on('click', '.card-tag', function () {
                ...
                let id = $(this).data('id');
                let price = recentView.formatPrice($(this).data('price'));
                ...
                if (liked) {
                    fetch(`/property-favorite/${pid}`, {
                        ...
                        .then(data => {
                            ...
                            recentView.favouritesProperties();
                        })
        ...
        recentViewProperties: function() {
            fetch('/dashboard/recent-view-property?limit=12')
                .then(response => response.json())
                .then(data => {
                    if (data.length > 0) {
                        return recentView.renderData(data, 'recent-view-properties', 'rv');
                    }
        ...
        renderData: function (data, element, id) {
            ...
            data.forEach((q, i) => {
                const newId = `${id}-${i}`;
                const tmpl = recentView.renderCard(q, newId);
        ...
        renderCard: function (q, i) {
            ...
            if (q.rented_room < q.total_room && q.is_co_living) {
                let price = recentView.formatPrice(q.co_living_min_price);
            ...
            if (q.available_room === q.total_room && q.is_entire_space) {
                ...
                let price = recentView.formatPrice(q.entire_space_min_price);
            ...
            let carousel = recentView.renderImage({ photos: q.photos, pageUrl: url, id: i });
            ...
            let liked = recentView.state.user.property_favorites;
